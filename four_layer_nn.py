...
np.random.seed(0)
syn0 = 2 * np.random.random((X_feature.shape[1], 5)) - 1
syn1 = 2 * np.random.random((syn0.shape[1]+1, 4)) - 1
syn2 = 2 * np.random.random((syn1.shape[1]+1, 1)) - 1
...
for i in xrange(60000):
	...
	l2_with_bias = np.concatenate((bias, l2), axis=1)

	l3 = nonlin(np.dot(l2_with_bias, syn2))
	
	if i % 5000 == 0:
		print "The error is: " + str(np.mean(np.abs(y - l3))) 
	...
	l2_delta = l2_error[:,1:] * nonlin(l2, True)
	...
	syn2 += np.dot(l2_with_bias.T, l3_delta)
